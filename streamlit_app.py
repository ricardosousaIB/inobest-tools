...
st.title("Agregador de Ficheiros Excel (via ZIP)")
...
if 'processed_data_zip' not in st.session_state:
    st.session_state.processed_data_zip = None
if 'processed_data_csv_preview' not in st.session_state:
    st.session_state.processed_data_csv_preview = None
if 'processing_done' not in st.session_state:
    st.session_state.processing_done = False
if 'arquivos_com_erro_state' not in st.session_state:
    st.session_state.arquivos_com_erro_state = []
if 'last_uploaded_file_id' not in st.session_state: # Para detetar novo upload
...
uploaded_zip_file = st.file_uploader(
    ...
    key="zip_uploader" # Adiciona uma chave para o uploader
)

# Lógica para resetar o estado quando um NOVO ficheiro é carregado
# Compara o ID do ficheiro carregado com o último ID conhecido
...
elif uploaded_zip_file is None and st.session_state.last_uploaded_file_id is not None:
...
if uploaded_zip_file is not None and not st.session_state.processing_done:
    excel_files_in_zip = []
    arquivos_com_erro = []
    ...
    try:
        with zipfile.ZipFile(uploaded_zip_file, 'r') as zf:
            ...
            if not excel_files_in_zip:
                ...
                st.session_state.processed_data_csv_preview = None
                st.session_state.arquivos_com_erro_state = []
                st.session_state.last_uploaded_file_id = None # Resetar o ID para permitir novo upload
    ...
    if header_written:
        ...
        st.session_state.arquivos_com_erro_state = arquivos_com_erro
        ...
        with st.spinner("A carregar dados agregados e a preparar para download..."):
            ...
            try:
                ...
                st.session_state.processed_data_csv_preview = df_final_preview.head()
                temp_csv_buffer.seek(0)
            except Exception as e:
        ...
        with st.spinner("A comprimir o CSV num ficheiro ZIP..."):
            ...
            st.session_state.processed_data_zip = zip_output_buffer.getvalue()
...
if st.session_state.processing_done:
    ...
    if st.session_state.processed_data_csv_preview is not None:
        st.subheader("Pré-visualização dos Dados Agregados (CSV):")
        st.dataframe(st.session_state.processed_data_csv_preview)
    
    if st.session_state.processed_data_zip is not None:
        st.download_button(
            label="Descarregar Resultado Agregado (resultado_agregado.zip)",
            data=st.session_state.processed_data_zip,
            file_name="resultado_agregado.zip",
            mime="application/zip"
        )
    
    if st.session_state.arquivos_com_erro_state:
        st.warning("Alguns ficheiros dentro do ZIP tiveram erros e não foram incluídos no resultado:")
        for erro in st.session_state.arquivos_com_erro_state:
            st.write(f"- {erro}")
